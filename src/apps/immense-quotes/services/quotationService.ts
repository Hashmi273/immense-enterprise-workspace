...
export const quotationService = {
  /**
   * Fetches saved quotations for an organization.
  ...
  async getQuotations(organizationId: string): Promise<SavedQuotationRecord[]> {
    try {
      ...
      if (error) {
        ...
        const local = localStorage.getItem(`immense_quotes_${organizationId}`);
        return local ? JSON.parse(local) : [];
      }

      // Update local backup
      if (data) {
        localStorage.setItem(`immense_quotes_${organizationId}`, JSON.stringify(data));
  ...
  async deleteQuotation(
    ...
    try {
      ...
      if (local) {
        ...
        localStorage.setItem(`immense_quotes_${organizationId}`, JSON.stringify(filtered));
      }
  ...
  updateLocalCache(orgId: string, record: SavedQuotationRecord) {
    try {
      const local = localStorage.getItem(`immense_quotes_${orgId}`);
      ...
      localStorage.setItem(`immense_quotes_${orgId}`, JSON.stringify(list));
    } catch {
